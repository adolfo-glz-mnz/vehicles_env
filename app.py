...
# Lógica para mostrar el histograma si la casilla de verificación está marcada
if hist_checkbox:
    st.write(
        'Mostrando histograma para el conjunto de datos de anuncios de venta de coches')
    fig = go.Figure(data=[go.Histogram(x=car_data['odometer'])])
    fig.update_layout(title_text='Distribución del Odómetro')
    st.plotly_chart(fig, use_container_width=True)

# Lógica para mostrar el gráfico de dispersión si la casilla de verificación está marcada
if scatter_checkbox:
    st.write(
        'Mostrando gráfico de dispersión para el conjunto de datos de anuncios de venta de coches')
    fig = go.Figure(
        data=[go.Scatter(x=car_data['odometer'], y=car_data['price'], mode='markers')])
    fig.update_layout(title_text='Relación entre Odómetro y Precio')
    st.plotly_chart(fig, use_container_width=True)
